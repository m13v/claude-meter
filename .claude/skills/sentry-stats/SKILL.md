---
name: sentry-stats
description: Use when the user asks "check Sentry", "Sentry analytics", "Sentry stats", "how many users yesterday", "DAU", "WAU", "who's on what version", "version adoption", "release health", "release adoption", "crash free", or anything about claude-meter usage from Sentry. Queries the mediar-n5/claude-meter Sentry project for unique hosts, version breakdown, release adoption, and (post v0.3.1) session-based DAU + crash-free metrics.
user_invocable: true
---

# Sentry Stats (claude-meter)

Pulls usage data from Sentry for the claude-meter desktop app. Two query lanes:
...
| Sentry project slug | `claude-meter` |
...
Today's date is in the user's CLAUDE.md system reminder; always convert relative ("yesterday", "this week") to absolute UTC before querying.
...
- **Releases** are reported as `claude-meter@<version>` because that is how `release:` is set in `src/bin/menubar.rs`. The leading prefix is meaningful for Sentry; do not strip it when filtering.
...
- If the app starts setting `sentry::User` with a stable anonymous install ID (e.g., a UUID persisted to `~/Library/Application Support/claude-meter/install_id`), swap the events lane to `count_unique(user)` and stop using hostname as a proxy.
- Releases data only becomes accurate once enough users upgrade past v0.3.1.

## Do NOT publish this skill

This skill hardcodes the `mediar-n5` org slug and the `claude-meter` project ID. It is project-internal and lives in this workspace's `.claude/skills/`. Do not run `publish-skill` against it; it is not useful to anyone outside this repo.
